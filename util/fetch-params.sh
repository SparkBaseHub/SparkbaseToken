...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/SparkParams"
    else
        PARAMS_DIR="$HOME/.spark-params"
    fi
fi
...
Failed to fetch the Sparkbase zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
Spark - fetch-params.sh

This script will fetch the Spark zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Spark zkSNARK parameters. Note that it is
