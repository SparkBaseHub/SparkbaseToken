...
"""Test sparkd with different proxy configuration.

Test plan:
- Start sparkd's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on sparkd side:
...
- Create sparkds that connect to them
- Manipulate the sparkds using addnode (onetry) an observe effects
...
class ProxyTest(SparkTestFramework):
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: sparkd's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
