...
def initialize_datadir(dirname, n):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    with open(os.path.join(datadir, "spark.conf"), 'w', encoding='utf8') as f:
        f.write("regtest=1\n")
        f.write("[regtest]\n")
...
def append_config(dirname, n, options):
    datadir = get_datadir_path(dirname, n)
    with open(os.path.join(datadir, "spark.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "spark.conf")):
        with open(os.path.join(datadir, "spark.conf"), 'r', encoding='utf8') as f:
            for line in f:
