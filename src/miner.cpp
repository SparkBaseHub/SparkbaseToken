...

    {
        ...
        if (fProofOfStake) {
          ...
          if (nHeight >= pCheckpointCache.first || pCheckpointCache.second.first != hashBlockLastAccumulated) {
              ...
              //printf("CreateNewBlock(): Height: %d, Zerocoin_Block_V2_Start: %d\n", nHeight, Params().Zerocoin_Block_V2_Start());
        ...
        LogPrintf("CreateNewBlock(): nHeight: %d GetHash: %s nBits: %d\n", pindexPrev->nHeight, pblock->GetHash().ToString().c_str(), pblock->nBits);
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        if (fProofOfStake) {
            ...
            while (vNodes.empty() || pwallet->IsLocked() || !fMintableCoins || (pwallet->GetBalance() > 0 && nReserveBalance >= pwallet->GetBalance()) || !masternodeSync.IsSynced()) {
                ...
                //LogPrintf("SparkBaseMiner: waiting for staking criterias \n");
        ...
        //printf("BASEMiner: fGenerateBitcoins %d\n", fGenerateBitcoins);
