...
UniValue movecmd(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw std::runtime_error(
            ...
            "3. amount            (numeric, required) Quantity of BASE to move between accounts.\n"
            ...
            "\nMove 0.01 BASE from timotei to akiko with a comment\n" +
...
UniValue getwalletinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee configuration, set in BASE/kB\n"
            "}\n"
...
UniValue listmintedzerocoins(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 2)
        throw runtime_error(
            ...
            "    \"zBASE ID\": \"xxx\",   (string) Pubcoin in hex format.\n"
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    set<CMintMeta> setMints = pwalletMain->zbaseTracker->ListMints(true, fMatureOnly, true);
    ...
    if (fVerbose) {
        for (const CMintMeta& m : setMints) {
            ...
            objMint.push_back(Pair("zBASE ID", m.hashPubcoin.GetHex()));     // PubCoin
...
UniValue mintzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. amount      (numeric, required) Enter an amount of Base to convert to zBASE\n"
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    ...
    return DoZbaseSpend(nAmount, fMintChange, fMinimizeChange, nSecurityLevel, vMintsSelected, address_str);
}
...
UniValue spendzerocoinmints(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nSpend zBASE mints to a BASE address.\n" +
            HelpRequiringPassphrase() + "\n"
    ...
    return DoZbaseSpend(nAmount, false, true, 100, vMintsSelected, address_str);
}


extern UniValue DoZbaseSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, const int nSecurityLevel, vector<CZerocoinMint>& vMintsSelected, std::string address_str)
{
