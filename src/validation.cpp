...
#include "zsparks/zsparksmodule.h"
...
/** Fees smaller than this (in uSparks) are considered zero fee (for relaying, mining and transaction creation)
...
CAmount GetBlockValue(int nHeight)
{
    ...
    if (nHeight > 525600)  return 13.5 * COIN;//Phase 9
    if (nHeight > 432000)  return 18   * COIN;//Phase 8
    if (nHeight > 345600)  return 22.5 * COIN;//Phase 7
    if (nHeight > 259200)  return 27   * COIN;//Phase 6
    if (nHeight > 172800)  return 31.5 * COIN;//Phase 1
...
{
    util::ThreadRename("spark-scriptch");
                                     ...
                                     std::set<CBigNum>& spent_serials)
{
    ...
    const bool zsparksActive = consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC);
    const bool publicZsparksActive = consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_PUBLIC);
    ...
    for (size_t i = 1; i < block.vtx.size(); i++) {
        ...
        for (const CTxIn& in: tx->vin) {
            ...
            if (isPublicSpend && (!publicZsparksActive || v5Active)) {
            ...
            if (isPrivZerocoinSpend && (!zsparksActive || publicZsparksActive)) {
            ...
            if (isPrivZerocoinSpend || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZSPARKSModule::ParseZerocoinPublicSpend(in, *tx, state, publicSpend)){
                        return false;
                    }
                    spend = publicSpend;
                } else {
        ...
        if (coinstake_in.IsZerocoinSpend()) {
            ...
            if (!ContextualCheckZerocoinSpend(coinstake, &spend, pindex->nHeight)) {
                return state.DoS(100,error("%s: main chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                        coinstake.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zsparks");
            }
        }
