...
bool GetKernelStakeModifier(uint256 hashBlockFrom, uint64_t& nStakeModifier, int& nStakeModifierHeight, int64_t& nStakeModifierTime, bool fPrintProofOfStake)
{
    ...
    if(pindexFrom->nHeight < 260) nStakeModifierSelectionInterval = 0;
    ...
    while (nStakeModifierTime < pindexFrom->GetBlockTime() + nStakeModifierSelectionInterval) {
        ...
        //LogPrintf("Get nStakeModifierTime: nHeight: %d, diff: %d\n", pindexNext->nHeight, (pindexFrom->GetBlockTime()-nStakeModifierTime));
        ...
        if (pindex->GeneratedStakeModifier()) {
            ...
            //LogPrintf("GeneratedStakeModifier nStakeModifierHeight:%d, nStakeModifier:%d\n", nStakeModifierHeight, pindex->nStakeModifier);
        }
    }
    nStakeModifier = pindex->nStakeModifier;
    return true;
...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZBaseStake(spend));
    } else {
        ...
        CBaseStake* baseInput = new CBaseStake();
        baseInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(baseInput);
    }
