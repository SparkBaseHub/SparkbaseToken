...
bool AppInit(int argc, char* argv[])
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/spark.conf are parsed in qt/spark.cpp's main()
    ...
    if (gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help") || gArgs.IsArgSet("-version")) {
        ...
        if (gArgs.IsArgSet("-version")) {
            ...
            strUsage += "\nUsage:  sparkd [options]                     Start " PACKAGE_NAME " Daemon\n";
    ...
    try {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                fprintf(stderr, "Error: Command line contains unexpected token '%s', see sparkd -h for a list of options.\n", argv[i]);
                return false;
            }
        }

        // -server defaults to true for sparkd but not for the GUI so do this here
...
int main(int argc, char* argv[])
{
    ...
    // Connect sparkd signal handlers
    noui_connect();
