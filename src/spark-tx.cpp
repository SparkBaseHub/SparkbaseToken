...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help")) {
        ...
        std::string strUsage = PACKAGE_NAME " spark-tx utility version " + FormatFullVersion() + "\n\n" +
                               "Usage:  spark-tx [options] <hex-tx> [commands]  Update hex-encoded spark transaction\n" +
                               "or:     spark-tx [options] -create [commands]   Create hex-encoded spark transaction\n" +
                               "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded spark transaction
