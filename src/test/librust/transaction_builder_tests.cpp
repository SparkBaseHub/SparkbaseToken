...
BOOST_AUTO_TEST_CASE(TransparentToSapling)
{
    ...
    // 0.5 t-SPARKS in, 0.4 shielded-SPARKS out, 0.1 t-SPARKS fee
...
BOOST_AUTO_TEST_CASE(SaplingToSapling)
{
    ...
    // --- 0.4 shielded-SPARKS in, 0.299 shielded-SPARKS out, 0.1 shielded-SPARKS fee, 0.001 shielded-SPARKS change (added to fee)
    ...
    // --- Now try with 1 shielded-SPARKS in, 0.5 shielded-SPARKS out, 0.1 shielded-SPARKS fee, 0.4 shielded-SPARKS change
...
BOOST_AUTO_TEST_CASE(FailsWithNegativeChange)
{
    ...
    // Generate a 0.5 SPARKS note
    auto testNote = GetTestSaplingNote(pa, 59990000);

    // Fail if there is only a Sapling output
    // 0.5 shielded-SPARKS out, 0.1 t-SPARKS fee
    ...
    // 0.5 t-SPARKS out, 0.1 t-SPARKS fee
    ...
    // 0.5 t-SPARKS out, 0.1 t-SPARKS fee, 0.59999 shielded-SPARKS in
    builder.AddSaplingSpend(expsk, testNote.note, testNote.tree.root(), testNote.tree.witness());
    BOOST_CHECK_EQUAL("Change cannot be negative", builder.Build().GetError());
