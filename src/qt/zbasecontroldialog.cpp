...
std::set<std::string> ZBaseControlDialog::setSelectedMints;
std::set<CMintMeta> ZBaseControlDialog::setMints;

bool CZBaseControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZBaseControlDialog::COLUMN_DENOMINATION || column == ZBaseControlDialog::COLUMN_VERSION || column == ZBaseControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZBaseControlDialog::ZBaseControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZBaseControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZBaseControlDialog::~ZBaseControlDialog()
{
    delete ui;
}

void ZBaseControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBaseControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZBaseControlWidgetItem* itemDenom(new CZBaseControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZBaseControlWidgetItem *itemMint = new CZBaseControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
...
void ZBaseControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBaseControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZBase_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBaseControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZBaseControlDialog::GetSelectedMints()
{
...
void ZBaseControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
