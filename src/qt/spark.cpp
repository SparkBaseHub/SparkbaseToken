...
#include "qt/spark/splash.h"
#include "qt/spark/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("spark-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in spark.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in spark.qrc)
...
public:
    ...
    /// Get window identifier of QMainWindow (SPARKGUI)
...
{
    window = new SPARKGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &SPARKGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &SPARKGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (!vpwallets.empty()) {
            ...
            window->addWallet(SPARKGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(SPARKGUI::DEFAULT_WALLET);
        }
#endif
        ...
        // SPARK: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &SPARKGUI::handlePaymentRequest);
        connect(window, &SPARKGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(spark_locale);
    Q_INIT_RESOURCE(spark);
    ...
    /// 6. Determine availability of data and blocks directory and parse spark.conf
    ...
    try {
        gArgs.ReadConfigFile(gArgs.GetArg("-conf", SPARK_CONF_FILENAME));
    } catch (const std::exception& e) {
        QMessageBox::critical(nullptr, PACKAGE_NAME,
    ...
    // spark: links repeatedly have their payment requests routed to this process:
