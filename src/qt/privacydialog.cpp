...
void PrivacyDialog::on_pushButtonZBaseControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZBaseControlDialog* zBaseControl = new ZBaseControlDialog(this);
    zBaseControl->setModel(walletModel);
    zBaseControl->exec();
}

void PrivacyDialog::setZBaseControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBaseSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBASE()
{
    ...
    if (!ZBaseControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZBaseControlDialog::GetSelectedMints();
    ...
    ZBaseControlDialog::setSelectedMints.clear();
    ui->labelzBaseSelected_int->setText(QString("0"));
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Base, ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zBASE "));
    ...
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zBASE </b> "));
