...
QString formatBalance(CAmount amount, int nDisplayUnit, bool isZsparks)
{
    return (amount == 0) ? ("0.00 " + BitcoinUnits::name(nDisplayUnit, isZsparks)) : BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, amount, false, BitcoinUnits::separatorAlways, true, isZsparks);
}

QString formatBalanceWithoutHtml(CAmount amount, int nDisplayUnit, bool isZsparks)
{
    return (amount == 0) ? ("0.00 " + BitcoinUnits::name(nDisplayUnit, isZsparks)) : BitcoinUnits::floorWithUnit(nDisplayUnit, amount, false, BitcoinUnits::separatorAlways, true, isZsparks);
}
...
{
    // return if URI is not valid or is no SPARK: URI
...
{
    // Convert spark:// to spark:
    //
    //    Cannot handle this later, because spark:// will cause Qt to see the part after // as host,
...
bool openConfigfile()
{
    return openFile(GetConfigFile(gArgs.GetArg("-conf", SPARK_CONF_FILENAME)), true);
}
...
{
    return GetAutostartDir() / "spark.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart) {
        ...
        if (!optionFile.good())
            return false;
        // Write a spark.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (gArgs.GetBoolArg("-testnet", false))
            optionFile << "Name=SPARK (testnet)\n";
        else if (gArgs.GetBoolArg("-regtest", false))
            optionFile << "Name=SPARK (regtest)\n";
        else
            optionFile << "Name=SPARK\n";
