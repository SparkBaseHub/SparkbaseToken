    ...
    for (unsigned int nOut = 0; nOut < wtx.tx->vout.size(); nOut++) {
        ...
        if (mine) {
            ...
            if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address)) {
                // Received by Sparkbase Token Address
    ...
    for (unsigned int nOut = 0; nOut < wtx.tx->vout.size(); nOut++) {
        ...
        if (ExtractDestination(txout.scriptPubKey, address)) {
            ...
            if (wtx.tx->HasZerocoinMintOutputs())
                continue;
            // Sent to Sparkbase Token Address
            ...
            if (sub.address.empty() && txout.scriptPubKey.StartsWithOpcode(OP_RETURN)) {
                ...
                // Burned SPARKS, op_return could be for a proposal/budget fee or another sort of data stored there.
...
void TransactionRecord::updateStatus(const CWalletTx& wtx, int chainHeight)
{
    ...
    // For generated transactions, determine maturity
            ...
            type == TransactionRecord::StakeZSPARKS ||
...
{
    return (type == TransactionRecord::StakeMint || type == TransactionRecord::Generated || type == TransactionRecord::StakeZSPARKS);
}
