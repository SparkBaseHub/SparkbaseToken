...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select SPARKS Outputs to Spend" :
            "Select Shielded SPARKS to Spend");
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show Sparkbase Token address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
