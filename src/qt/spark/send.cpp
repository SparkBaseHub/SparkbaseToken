...
#include "qt/spark/send.h"
#include "qt/spark/forms/ui_send.h"
#include "qt/spark/addnewcontactdialog.h"
#include "qt/spark/qtutils.h"
#include "qt/spark/sendchangeaddressdialog.h"
#include "qt/spark/optionbutton.h"
#include "qt/spark/sendconfirmdialog.h"
#include "qt/spark/guitransactionsutils.h"
#include "qt/spark/loadingdialog.h"
...
SendWidget::SendWidget(SPARKGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-sparks");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onSPARKSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onSPARKSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent SPARKS.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res) {
                inform(tr("Error generating address to shield SPARKS"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent SPARKS to shield."));
    }
}
...
void SendWidget::onSPARKSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto sparksAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(sparksAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(sparksAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(sparksAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
