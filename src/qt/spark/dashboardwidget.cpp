...
#include "qt/spark/dashboardwidget.h"
#include "qt/spark/forms/ui_dashboardwidget.h"
#include "qt/spark/sendconfirmdialog.h"
#include "qt/spark/txrow.h"
...
DashboardWidget::DashboardWidget(SPARKGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareSpark, "square-chart-sparks");
    setCssProperty(ui->labelSquarezSparks, "square-chart-zsparks");
    setCssProperty(ui->labelSpark, "text-chart-sparks");
    setCssProperty(ui->labelZsparks, "text-chart-zsparks");
    ...
    setCssProperty(ui->labelAmountSpark, "text-stake-sparks-disable");
    setCssProperty(ui->labelAmountZsparks, "text-stake-zsparks-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &SPARKGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isSpark = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZSPARKS;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair SPARKS, zSPARKS
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            sparks = (pair.first != 0) ? pair.first / 100000000 : 0;
            zsparks = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalSpark += pair.first;
            chartData->totalZsparks += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesSparks.append(sparks);
        chartData->valueszSparks.append(zsparks);

        int max = std::max(sparks, zsparks);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(41,158,192));
    set1->setColor(QColor(21,48,87));
    ...
    set0->append(chartData->valuesSparks);
    set1->append(chartData->valueszSparks);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalSpark > 0 || chartData->totalZsparks > 0) {
        setCssProperty(ui->labelAmountSpark, "text-stake-sparks");
        setCssProperty(ui->labelAmountZsparks, "text-stake-zsparks");
    } else {
        setCssProperty(ui->labelAmountSpark, "text-stake-sparks-disable");
        setCssProperty(ui->labelAmountZsparks, "text-stake-zsparks-disable");
    }
    forceUpdateStyle({ui->labelAmountSpark, ui->labelAmountZsparks});
    ui->labelAmountSpark->setText(GUIUtil::formatBalance(chartData->totalSpark, nDisplayUnit));
    ui->labelAmountZsparks->setText(GUIUtil::formatBalance(chartData->totalZsparks, nDisplayUnit, true));
