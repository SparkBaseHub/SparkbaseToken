...
#include "qt/spark/forms/ui_snackbar.h"
#include "qt/spark/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(SPARKGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &SPARKGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
