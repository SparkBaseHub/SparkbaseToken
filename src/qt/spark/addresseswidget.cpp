...
#include "qt/spark/addresseswidget.h"
#include "qt/spark/forms/ui_addresseswidget.h"
#include "qt/spark/addressfilterproxymodel.h"
#include "qt/spark/addresslabelrow.h"
...
AddressesWidget::AddressesWidget(SPARKGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto sparksAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(sparksAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(sparksAdd);
        if (walletModel->updateAddressBookLabels(sparksAdd, label.toUtf8().constData(),
