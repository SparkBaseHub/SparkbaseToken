...
#include "qt/spark/topbar.h"
#include "qt/spark/forms/ui_topbar.h"
#include "qt/spark/lockunlock.h"
#include "qt/spark/qtutils.h"
#include "qt/spark/receivedialog.h"
#include "qt/spark/loadingdialog.h"
#include "askpassphrasedialog.h"

#include "bitcoinunits.h"
#include "qt/spark/balancebubble.h"
...
TopBar::TopBar(SPARKGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopSpark, ui->labelAmountTopShieldedSparks}, "amount-small-topbar");
    setCssProperty({ui->labelAmountSpark}, "amount-topbar");
    setCssProperty({ui->labelPendingSpark, ui->labelImmatureSpark}, "amount-small-topbar");
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("SPARK")
    updateDisplayUnit();
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // SPARKS Total
    QString totalSpark = GUIUtil::formatBalance(newBalance.balance, nDisplayUnit);
    ...
    // SPARKS
    // Top
    ui->labelAmountTopSpark->setText(totalTransparent);
    ui->labelAmountTopShieldedSparks->setText(totalShielded);
    // Expanded
    ui->labelAmountSpark->setText(totalSpark);
    ui->labelPendingSpark->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance + newBalance.unconfirmed_shielded_balance, nDisplayUnit));
    ui->labelImmatureSpark->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
}
