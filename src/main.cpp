...
/** Fees smaller than this (in ubase) are considered zero fee (for relaying and mining)
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zbase");
            }
        } else {
            LOCK(pool.cs);
...
int64_t GetBlockValue(int nHeight)
{
    ...
    //Phase 1
    else if (nHeight <= 43200 && nHeight > 1000) {
        nSubsidy = 1 * COIN;
    }
    //Phase 2
    ...
    //Phase 3
    else if (nHeight <= 129600 && nHeight > 86400) {
        nSubsidy = 5 * COIN;
    }
    //Phase 4
    else if (nHeight <= 172800 && nHeight > 129600) {
        nSubsidy = 10 * COIN;
    }
    //Phase 5
    ...
    //Phase 7
    else if (nHeight <= 432000 && nHeight > 345600) {
        nSubsidy = 50 * COIN;
    }
    //Phase 8
    else if (nHeight <= 525600 && nHeight > 432000) {
        nSubsidy = 60 * COIN;
    }
    //Phase 9
    else if (nHeight <= 777600 && nHeight > 525600) {
        nSubsidy = 50 * COIN;
    }
    //Phase 10
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZBASEStake)
{
    int64_t ret = 0;
  	...
  	if (nHeight <= 1000) {
  		  ...
  		  ret = blockValue  / 100 * 70;// 70% for Masternodes
  	}
...
bool CheckInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, bool fScriptChecks, unsigned int flags, bool cacheStore, std::vector<CScriptCheck>* pvChecks)
{
    if (!tx.IsCoinBase() && !tx.IsZerocoinSpend()) {
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++) {
            ...
            // If prev is stake coin, check that it's matured
            // maturity increased after block 23000 from 4 to 10
            if (coins->IsCoinBase() || coins->IsCoinStake()) {
                int maturity = (nSpendHeight<23000)? 3 : Params().COINBASE_MATURITY();
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (!UpdateZBASESupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zBASE supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zBaseSpent: %s\n",
...
//              FormatMoney(nFees), FormatMoney(pindex->nMint), FormatMoney(nAmountZerocoinSpent));
    ...
    //printf("IsBlockValueValid checked nHeight %i, nExpectedMint %i, nMint %i\n", pindex->pprev->nHeight, nExpectedMint, pindex->nMint);
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    //printf("CheckBlock: block=%s  is proof of work=%d\n", block.GetHash().ToString().c_str(), block.IsProofOfWork());

    // Check that the header is valid (particularly PoW).  This is mostly
    // redundant with the call in AcceptBlockHeader.
    bool posCheck = (block.IsProofOfWork() && fCheckPOW);
    if (!CheckBlockHeader(block, state, posCheck))
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    ...
    if (CZBaseStake* zBASE = dynamic_cast<CZBaseStake*>(stake)) {
...
bool CVerifyDB::VerifyDB(CCoinsView* coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        if (nCheckLevel >= 1 && !CheckBlock(block, state, block.IsProofOfWork()))
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        if (vAddr.size() > 1000) {
            LOCK(cs_main);
            LogPrintf("Misbehaving: message addr size() = %u", vAddr.size());
        ...
        if (vInv.size() > MAX_INV_SZ) {
            LOCK(cs_main);
            LogPrintf("Misbehaving: message inv size() = %u", vInv.size());
            Misbehaving(pfrom->GetId(), 20);
        ...
        if (vInv.size() > MAX_INV_SZ) {
            LOCK(cs_main);
            LogPrintf("Misbehaving: message getdata size() = %u", vInv.size());
            Misbehaving(pfrom->GetId(), 20);
    ...
    else if (strCommand == "headers" && Params().HeadersFirstSyncingActive() && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (nCount > MAX_HEADERS_RESULTS) {
            LOCK(cs_main);
            LogPrintf("Misbehaving: headers message size = %u", nCount);
            Misbehaving(pfrom->GetId(), 20);
        ...
        BOOST_FOREACH (const CBlockHeader& header, headers) {
            ...
            if (pindexLast != NULL && header.hashPrevBlock != pindexLast->GetBlockHash()) {
                LogPrintf("Misbehaving: non-continuous headers sequence");
                Misbehaving(pfrom->GetId(), 20);
